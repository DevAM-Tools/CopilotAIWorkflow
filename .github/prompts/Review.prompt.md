...
Exact command with build config (`-c Release`) and a framework-appropriate test filter expression. State the expected outcome: exit code, passing test count, or specific observable state.
...
Exact command with build config (`-c Release`) confirming the change is correctly applied and the build remains clean.
...
Exact test command with build config (`-c Release`) confirming behaviour is unchanged. State passing test count or specific assertions.
