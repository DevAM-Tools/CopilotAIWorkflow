...
- Use `[Arguments(...)]` or `[MethodDataSource(nameof(...))]` for data-driven scenarios; enumerate every corner case explicitly as a separate `[Arguments]` row rather than relying on a single representative value.
