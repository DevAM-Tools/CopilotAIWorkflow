...
public static class CoberturaReader
{
    ...
    {
        return _TryReadCore(path, scope: null, out document, out error);
    }

    /// <summary>Attempts to read a Cobertura file retaining only scoped package lines.</summary>
    /// <param name="path">Path to Cobertura XML.</param>
    /// <param name="scope">Package scope filter.</param>
    /// <param name="document">Parsed scoped document when successful.</param>
    /// <param name="error">Error message when unsuccessful.</param>
    /// <returns><see langword="true"/> when parsing succeeds.</returns>
    public static bool TryReadScoped(string path, BranchGapScope scope, out Models.CoberturaDocument? document, out string? error)
    {
        ArgumentNullException.ThrowIfNull(scope);
        return _TryReadCore(path, scope, out document, out error);
    }

    private static bool _TryReadCore(string path, BranchGapScope? scope, out Models.CoberturaDocument? document, out string? error)
    {
        document = null;
        error = null;
        ...
        try
        {
            ...
            return _TryParse(reader, path, scope, out document, out error);
        }
    ...
    private static bool _TryParse(XmlReader reader, string path, BranchGapScope? scope, out Models.CoberturaDocument? document, out string? error)
    {
        document = null;
        error = null;
        ...
        bool currentPackageInScope = scope is null;
        ...
        while (reader.Read())
        {
            ...
            switch (reader.Name)
            {
                ...
                case "package":
                    ...
                    currentPackageInScope = scope is null
                        || (!string.IsNullOrEmpty(currentPackage) && scope.IncludesPackage(currentPackage));
                ...
                case "method":
                    ...
                    break;
                case "line":
                    if (!currentPackageInScope || string.IsNullOrEmpty(currentClassFile))
                    {
                        break;
                    }
