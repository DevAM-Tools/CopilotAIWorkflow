...
internal static class ProjectGraphBuilder
{
    ...
    public static bool TryBuild(CliOptions options, out IReadOnlyList<ProductionProjectRecord> projects, out string? error)
    {
        ...
        TestProjectIndex testProjectIndex = TestProjectIndex.Build(repositoryRoot);
        ...
        if (records.Count == 0)
        {
            ...
            if (_AllGapCandidatesAreExecutable(candidatePaths, repositoryRoot))
            {
    ...
    private static bool _AllGapCandidatesAreExecutable(IReadOnlyList<string> candidatePaths, string repositoryRoot)
    {
