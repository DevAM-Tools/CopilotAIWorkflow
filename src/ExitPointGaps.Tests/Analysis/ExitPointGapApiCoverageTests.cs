...
public sealed class ExitPointGapApiCoverageTests
{
    ...
    [Test]
    public async Task ExitPointGapReportFormatter_CompactAndText_CoverOptionalFields()
    {
        ExitPointGapReport emptyPassed = new ExitPointGapReport(
            new ExitPointGapSummary(1d, 0, 0, 0, true, true),
            [],
            []);

        ExitPointGapReport withGaps = new ExitPointGapReport(
            new ExitPointGapSummary(0.5d, 1, 1, 2, false, false),
            [new ExitCoverageGap(1, "id", "a.cs", 1, 1, "Return", "M", 0, null)],
            [new BranchGap(2, "Sample", "Foo", "b.cs", 2, 0, 0.5d, 1, null, null)]);

        string emptyText = ExitPointGapReportFormatter.ToText(emptyPassed);
        string compactNoMethod = ExitPointGapReportFormatter.ToCompact(withGaps);
        string textWithSections = ExitPointGapReportFormatter.ToText(withGaps);
    ...
    {
        await Assert.That(BranchGapScope.Default.IncludesPackage("CSharpStyleChecker")).IsTrue();
        await Assert.That(BranchGapScope.Default.IncludesPackage("ExitPoints")).IsTrue();
        await Assert.That(BranchGapScope.Default.IncludesPackage("ExitPointGaps")).IsTrue();
    ...
    [Test]
    public async Task ExitPointGapReportBuilder_NullArguments_Throw()
    {
        await Assert.That(() => ExitPointGapReportBuilder.Build(null!, [], [], @"C:\repo", false))
            .Throws<ArgumentNullException>();
        await Assert.That(() => ExitPointGapReportFormatter.ToCompact(null!)).Throws<ArgumentNullException>();
